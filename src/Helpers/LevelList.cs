...
namespace Leisn.MarkdigToc.Helpers
{
    /// <summary>
    /// Sort and merge with level
    /// </summary>
    /// <typeparam name="T">LevelList`T</typeparam>
    public class LevelList<T> where T : LevelList<T>, new()
    {
        /// <summary>
        /// Current Level
        /// </summary>
        ...
        T findChildLevelLessThan(int level, T current)
        {
            if (current._current == null || current._current.Level >= level)
                return current;
            return findChildLevelLessThan(level, current._current);
        }

        T? findParentLevelLessThan(int level, T current)
        {
            if (current.Parent == null)
                return null;
            if (current.Parent.Level < level)
                return current.Parent;
            return findParentLevelLessThan(level, current.Parent);
        }
        ...
        public void Append(T item)
        {
            ...
            if (item.Level > _current.Level)
            {
                ...
                var found = findChildLevelLessThan(item.Level, _current);
            ...
            var parent = findParentLevelLessThan(item.Level, _current);
        ...
        protected virtual void MargeSiblings(T parent, T add)
        {
            ...
            for (int i = 0; i < parent.Count; i++)
            {
                ...
                {
                    endBefore = i;
                    list.Add(new int[] { startAt, endBefore });
                    startAt = -1;
                }
            }
